...
private:
    ...
    public:
        ...
        Matrix::Row operator+(Matrix<M,N>::Row &a);
...
public:
    ...
    friend std::ostream& operator<< <> (std::ostream &cout, Matrix<Y,X> &a);
};
...
template<int Y, int X>
template<int M,int N>
Matrix<Y,X>::Row Matrix<Y,X>::Row::operator+(Matrix<M,N>::Row a)
{
...
template<int Y, int X>
std::ostream& operator<<(std::ostream &cout, Matrix<Y,X> &a)
{
    for(int i=1;i<=Y;i++)
    {
        cout << "[";
